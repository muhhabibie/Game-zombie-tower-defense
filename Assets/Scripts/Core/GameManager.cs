...
namespace LastBastion.Core
{
    ...
    public class GameManager : MonoBehaviour
    {
        ...
        public LastBastion.Enemies.EnemySpawner enemySpawner;
        public LastBastion.Towers.TowerManager towerManager;
        public LastBastion.Skills.SkillManager skillManager;
        public LastBastion.Upgrades.UpgradeManager upgradeManager;
        public LastBastion.Events.RandomEventManager randomEventManager;
        ...
        private void EnterPreparation()
        {
            ...
            hud?.SetPhase("Prep");
            towerManager?.EnablePlacement(true);
            skillManager?.EnableInput(true);
        ...
        public void StartWave()
        {
            ...
            currentWave++;
            hud?.SetPhase($"Wave {currentWave}");
            towerManager?.EnablePlacement(false);

            int enemyCount = Mathf.RoundToInt(enemyCountByWave.Evaluate(currentWave));
            float hpMult = enemyHpMultiplierByWave.Evaluate(currentWave);

            isSpawning = true;
            enemySpawner.SpawnWave(enemyCount, hpMult, () =>
            {
                isSpawning = false;
            });
            ...
            // Chance random event
            randomEventManager?.TryTriggerRandomEvent(currentWave);
            StartCoroutine(MonitorWaveEnd());
        }

        private IEnumerator MonitorWaveEnd()
        {
            // Wait until all enemies are cleared
            while (isSpawning || enemySpawner.ActiveEnemyCount > 0)
            {
        ...
        private void EnterUpgradePhase()
        {
            ...
            upgradeManager?.ShowChoices(3, OnUpgradeSelected);
        }
        ...
        public void TriggerDefeat()
        {
            ...
            hud?.SetPhase("Defeat");
            OnDefeat?.Invoke();
            // TODO: Show defeat UI, offer restart (permadeath)
        }
    }
}